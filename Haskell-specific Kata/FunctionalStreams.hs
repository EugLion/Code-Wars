module FunctionalStreamsKata where
...
filterS f (x:>xs) = let fs = filterS f xs in if f x then x :> fs else fs

takeS :: Int -> Stream a -> [a]
takeS n (x:>xs) = if n <= 0 then [] else x : takeS (n - 1) xs

dropS :: Int -> Stream a -> Stream a
dropS n acc@(_:>xs) = if n == 0 then acc else dropS (n - 1) xs
