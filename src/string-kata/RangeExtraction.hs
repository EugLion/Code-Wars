...
groupSucc [] = []
groupSucc xs = let (ys, zs) = go xs in ys : groupSucc zs
 where
  go [x     ] = ([x], [])
  go (x:y:ys) = if y == succ x
    then let r = go (y : ys) in (x : fst r, snd r)
    else ([x], y : ys)

-- | Some cool pointfree stuff
range' :: [Int] -> String
range' = intercalate "," . map toRange . groupBy ((==) `on` uncurry (-)) . zip
  [1 ..]
 where
  toRange [(_, x)]         = show x
...
range'' = intercalate "," . map formatRange . toRanges []
 where
  ...
  toRanges ((a, b):rs) (x:xs) = if x == b + 1
    then toRanges ((a, x) : rs) xs
    else toRanges ((x, x) : (a, b) : rs) xs
  formatRange (a, b) | a == b     = show a
                     | a + 1 == b = show a ++ "," ++ show b
